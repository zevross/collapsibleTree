...
collapsibleTree.Node <- function(
  ...
  # message("Starting `collapsibleTree.Node` ", Sys.time())

  if (identical(default_tree, FALSE)) {
    ...
    # x <- readRDS(system.file("extdata", "default_tree.RDS", package = "DEAP"))
    # x <- readRDS(system.file("extdata", default_tree, package = "DEAP"))
    x <- readRDS(default_tree)
  }

  # message("Before `createWidget` ", Sys.time())
